...
Article.numWordsAll = () => {
  return Article.all.map( (article) => article.body.split(' ').length).reduce((acc, count) => acc + count, 0);
};
...
Article.numWordsByAuthor = () => {
  return Article.allAuthors().map(author => {
    ...
    const articles = Article.all.filter( article => article.author === author );
    const numWords = articles.map( (article) => article.body.split(' ').length).reduce((acc, count) => acc + count, 0);
