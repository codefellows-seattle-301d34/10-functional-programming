...
Article.loadAll = articleData => {
  ...
  Article.all = articleData.map((element) => new Article(element));
  /* OLD forEach():
...
Article.numWordsByAuthor = () => {
  return Article.allAuthors().map(author => {
    return {
      ...
      //   obj.bodyLen = element.body.length;
      //   return obj;
      // }).sort((a,b) => {
      ...
      //   }
      //   return 0;
      // }).reduce((prevAns, current, index) => {
      //   const length = prevAns.length;
      //   if(length === 0 || prevAns[length - 1].author !== current.author) {   
      //   }
      // }, [])
    }
  })
