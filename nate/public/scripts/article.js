...
(function(module) {
  ...
  Article.numWordsAll = () => {
    return Article.all.map(article => article.body.split(' ').length).reduce((acc, inc) => acc + inc);
  };

  // Hint: Make sure to return an array and avoid duplicates.
  Article.allAuthors = () => {
    return Article.all.map(article => article.author).reduce((acc, inc) => {
      if (!acc.includes(inc)) {
        acc.push(inc);
      }
      return acc;
  ...
  Article.numWordsByAuthor = () => {
    return Article.allAuthors().map(author => {
      ...
      let totalWordCount = Article.all.filter(article => article.author === author).map(article => article.body.split(' ').length).reduce((acc, inc) => acc + inc);
      return {
        name: author,
        // Hint: you will need to chain some combination of .filter(), .map(), and .reduce() to get the value of the numWords property
        numWords: totalWordCount,
      }
    })
  };
